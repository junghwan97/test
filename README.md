...
###🚀 기존 문제점 (AS-IS)
### 1️⃣ **비관적 락(Pessimistic Lock) 기반 재고 관리**
- **DB 트랜잭션에서 `SELECT ... FOR UPDATE`를 사용하여 재고 차감**  
- **트랜잭션이 끝날 때까지 다른 요청이 블록됨**  
- **동시 요청이 많아질수록 DB 락 경합으로 인해 성능 저하 발생**

### 🔥 **발생한 문제**
1. **트랜잭션이 길어질수록 성능 저하 심화**  
2. **대량 요청이 발생하면 처리량(TPS)이 급격히 감소**  
3. **DB 락 대기로 인해 일부 요청은 Timeout 발생**  

---

## 🔧 **개선 과정 (TO-BE)**
### ✅ **1. Redis 분산락 적용**
- **Redisson `tryLock()`을 활용하여 락 경합 감소**
- **DB 부하를 줄이고 동시 요청 처리량 증가**
- ⚠️ 하지만, **Redis와 DB의 정합성 불일치 문제 발생**

### ✅ **2. Lua 스크립트 적용 (최적화)**
- **락 없이 Redis에서 원자적으로 재고 차감**
- **DB 업데이트 실패 시 Redis 재고 복구**
- **네트워크 왕복 없이 빠른 재고 처리 가능**

## 📊 성능 비교

아래 표는 **비관적 락 → Redis 분산락 → Lua 스크립트 적용 후 성능 변화 비교표**입니다.

| 방식 | 재고 정합성 | 평균 응답 시간(ms) | TPS(처리량) | DB 부하 감소율 |
|:----:|:------:|:------:|:------:|:------:|
| **비관적 락 사용** | ✅ (정합성 유지) | `{{ 기존 평균 응답 시간 }}` | `{{ 기존 TPS }}` | 0% |
| **Redis 분산락 적용** | ⚠️ (불일치 가능) | `{{ Redis 락 응답 시간 }}` | `{{ Redis TPS }}` | `{{ Redis DB 감소율 }}` |
| **Lua 스크립트 적용** | ✅ (완벽 유지) | `{{ Lua 응답 시간 }}` | `{{ Lua TPS }}` | `{{ Lua DB 감소율 }}` |
...
## 📌 결론
비관적 락에서 Redis 및 Lua 스크립트를 활용하여 **고성능 재고 관리 시스템**을 구현한 과정입니다.

### 🔥 개선 효과 정리
- **비관적 락 사용 시**  
  - 정합성은 유지되지만 **DB 부하 증가 및 동시 요청 처리량 감소**  
  - 락 경합으로 인해 대기 시간 증가  
...
### 🎯 최종 결론
✅ **비관적 락을 사용하면 정합성을 보장할 수 있지만 성능 저하 발생**  
