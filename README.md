...
# **⚡성능 개선**
- **Redis 분산락을 통한 재고 동시성 제어**
  - 문제: 높은 동시성 환경에서 재고 관리의 정합성과 성능 확보 필요
  - 해결: Redis 분산락 도입으로 빠른 락 획득/해제 구현
  - 효과: DB 부하 감소 및 트랜잭션 처리 속도 향상

- **Spring Cloud Gateway를 활용한 로드밸런싱**
  - 문제: 마이크로서비스 간 트래픽 분산 필요
  - 해결: API Gateway를 통한 효율적인 라우팅 구현
  - 효과: 서비스 부하 분산 및 가용성 향상

- **캐싱 전략 도입**
  - 문제: 반복적인 DB 조회로 인한 성능 저하
  - 해결: Redis를 활용한 상품 정보 캐싱
  - 효과: 응답 시간 단축 및 DB 부하 감소
